<template>
  <a-card>
    ...
    <a-button type="primary" style="margin-bottom:12px" @click="()=>message.info('新建功能开发中')">新建</a-button>
...
async function loadData(){loading.value=true;const p=new URLSearchParams({page:String(pagination.current),size:String(pagination.pageSize)})
  const res=await fetch(`/api/${isSupplier.value?'suppliers':'customers'}?${p}`);const d=await res.json()
