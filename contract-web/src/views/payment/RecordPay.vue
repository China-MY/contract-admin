<template>
  <a-card><template #title><span style="font-size:18px;font-weight:bold">付款记录</span></template>
    <a-form layout="inline" :model="sf" style="margin-bottom:16px">
      ...
      <a-form-item label="付款日期"><a-date-picker v-model:value="sf.date" style="width:160px" /></a-form-item>
    ...
    <div style="margin-bottom:12px"><a-button type="primary" @click="()=>message.info('新建付款记录功能开发中')">新建付款记录</a-button><a-button style="margin-left:8px" @click="()=>message.info('导入功能开发中')">导入</a-button></div>
...
const columns=[
  ...
  {title:'发票号码',dataIndex:'invoiceNo',width:130},{title:'付款单号',dataIndex:'recordNo',width:130},
  {title:'付款日期',dataIndex:'recordDate',width:100},{title:'付款金额',key:'amount',width:110},
  {title:'付款状态',key:'status',width:90},{title:'付款方式',dataIndex:'method',width:100},
  {title:'付款账户',dataIndex:'account',width:120},{title:'费用类型',dataIndex:'expenseType',width:90},
  {title:'付款方',dataIndex:'payer',width:120},{title:'收票方',dataIndex:'payee',width:120},
...
async function loadData(){
  ...
  const res=await fetch(`/api/payment-records?direction=pay&${params}`);const d=await res.json()
