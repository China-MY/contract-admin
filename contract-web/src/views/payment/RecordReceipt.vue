    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" width="65%" :footer="null" destroyOnClose>
      <a-form v-if="modalVisible" :model="form" layout="vertical" @finish="handleSave">
        <a-row :gutter="24">
          ...
          <a-col :span="8"><a-form-item label="付款方"><SelectCreate v-model="form.payer" :options="partyOptions" placeholder="搜索客户或供应商" @create="(n:string)=>form.payer=n" /></a-form-item></a-col>
          <a-col :span="8"><a-form-item label="收款方"><SelectCreate v-model="form.payee" :options="companyOptions" placeholder="选择我方公司" @create="(n:string)=>form.payee=n" /></a-form-item></a-col>
