    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" width="55%" :footer="null" destroyOnClose>
      <a-form v-if="modalVisible" :model="form" layout="vertical" @finish="handleSave">
        <a-row :gutter="24">
          ...
          <a-col :span="12"><a-form-item label="付款方" name="payer"><a-input v-model:value="form.payer" /></a-form-item></a-col>
          <a-col :span="12"><a-form-item label="收款方" name="payee"><a-input v-model:value="form.payee" /></a-form-item></a-col>
...
async function handleSave(){saving.value=true;const payload={...form,direction:'receipt',planCode:'PS-' + Date.now()}
