  ...
  <a-card>
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" width="60%" :footer="null" destroyOnClose>
      <a-form v-if="modalVisible" :model="formModel" layout="vertical" @finish="handleSave">
        <a-row :gutter="24">
          <a-col :span="12"><a-form-item label="项目编号"><a-input v-model:value="formModel.projectNo" /></a-form-item></a-col>
          <a-col :span="12"><a-form-item label="项目名称"><a-input v-model:value="formModel.projectName" /></a-form-item></a-col>
          ...
          <a-col :span="12"><a-form-item label="项目经理"><a-input v-model:value="formModel.manager" /></a-form-item></a-col>
...
const formModel=ref<any>({projectNo:'',projectName:'',projectType:'',status:'init',manager:'',budgetAmount:0})
...
async function loadData(){loading.value=true;const p=new URLSearchParams({page:String(pagination.current),size:String(pagination.pageSize)})
  const res=await fetch(`/api/projects?${p}`);const d=await res.json()
...
function showAdd(){currentRecord.value=null;formModel.value={projectNo:'',projectName:'',projectType:'',status:'init',manager:'',budgetAmount:0};modalTitle.value='新建项目';modalVisible.value=true}
function edit(r:any){currentRecord.value={...r};formModel.value={...r};modalTitle.value='编辑项目';modalVisible.value=true}
async function handleSave(){modalVisible.value=false;loadData()}
</script>
